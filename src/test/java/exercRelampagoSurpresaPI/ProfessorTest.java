...
class ProfessorTest {
    ...
    @Test
    void deveRetornarNomeCoordenadorCurso2() {
        Professor coordenador = new Professor("Antonio");
        Curso cursoMock = createMock(Curso.class);
        expect(cursoMock.getNomeCoordenador()).andReturn("Antonio");
        replay(cursoMock);
        coordenador.setCurso(cursoMock);

        assertEquals("Antonio", coordenador.getNomeCoordenador());
    }
